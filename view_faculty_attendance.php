...
include_once 'dbconn.php';
            ...
            $reg=$_POST['faculty_username'];
                if($deptValue == 'CSE' && $reg){
                    echo '<table style="margin-top:-60%;width: 100%;border-collapse: collapse;text-align: center;">';
                    echo '<tr>';
                    ...
                    echo '<th style="text-align:center;height:25px;width: 30%; border-style: groove;background-color: #F1C40F;color: white">'."<h5>Username</h5>".'</th>';
                    ...
                    echo '</tr>';
                    $query = "SELECT * from faculty WHERE faculty_username='$reg'";
                    $rs=mysqli_query($link,$query);
                    if($total_found=mysqli_num_rows($rs)){
                        while ($row = mysqli_fetch_array($rs)){
                            $id=  $row['faculty_dept'];
                            $no[]=  $row['faculty_username'];
                            echo '<tr style="color:black;">';
                            echo '<td style="height:25px;background-color:white;width: 25%;border-style: groove;">'.$row['faculty_fname']." ".$row['faculty_lname'].'</td>';
                            echo '<td style="height:25px;background-color:white;width: 30%; border-style: groove;">'.$row['faculty_username'].'</td>';
